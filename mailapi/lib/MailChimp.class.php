...
class MailChimp
{
    ...
    private $version = "1.3";
    private $export_version = "1.0";
    public $errorMessage;
    public $errorCode;

    private $apiUrl;
    private $exportApiUrl;
    private $timeout = 300;
    ...
    function __construct($apikey, $secure=false)
    {
        ...
        $this->exportApiUrl = parse_url("http://api.mailchimp.com/export/" . $this->export_version . "/");
        $this->api_key = $apikey;
    }
    ...
    function callExportServer($method, $params)
    {
        ...
        $params['apikey'] = $this->api_key;
        $url = "http://" . $dc . "." . $this->exportApiUrl["host"] . $this->exportApiUrl['path'] . $method . "/";
        $ch = curl_init($url);
