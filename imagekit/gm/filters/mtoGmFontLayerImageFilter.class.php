...
class mtoGmFontLayerImageFilter extends mtoAbstractImageFilter
{
  ...
  function apply(lmbAbstractImageContainer $container)
  {
    ...
    $canvas_all->newImage($canvas_all_width,$canvas_all_height,   "transparent");
    ...
    foreach($lines as $ind=> $line)
    {
        ...
        //var_dump($metrics);
        $canvas->newImage($metrics['textWidth']+5,$metrics['textHeight'],  "transparent");
        $canvas->annotateImage($draw, 0, $metrics['textHeight']+$metrics['descender'], 0, $line->getFontText());

        $canvas->resizeImage($box_width+3,$box_height,Gmagick::FILTER_CATROM,1);
        ...
        $canvas_all->compositeImage($canvas, Gmagick::COMPOSITE_OVER, $offset_l,$offset_top);
    ...
    if ($angle !== 0)
    {
        $canvas_all->rotateImage(new GmagickPixel('transparent'), $angle);
    }

    

    
    $container->getResource()->compositeImage($canvas_all, Gmagick::COMPOSITE_OVER, $offset_all_left,$offset_all_top);
