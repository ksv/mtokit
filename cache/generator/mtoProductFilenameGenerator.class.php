...
class mtoProductFilenameGenerator extends mtoCommonFilenameGenerator
{
    ...
    function parse($args = array())
    {
        ...
        if (strpos($params['side'], 'o') === 0)
        {
            $params['side'] = substr($params['side'], 1);
            $params['orig'] = 1;
        }
...
//        }
    ...
    function extract_args($args)
    {
        ...
        $opt = $args['instance']->option_by_side($side);
        if ($args['instance']->isCanvas())
        {
            $args['use_3dmask'] = 1;
        }
        if ($args['instance']->isCommon())
        {
            if (!empty($opt['opt_mask_3d']) && file_exists(mtoToolkit :: instance()->getFilename("item", $opt['opt_mask_3d'], $opt['opt_item'])))
            {
                $args['use_3dmask'] = 1;
            }
        }
    ...
    protected function createParts($args = array())
    {
        ...
        $args['option_id'] = !empty($o['opt_id']) ? $o['opt_id'] : '0';
        if (!isset($args['instance']))
        {
            //mtoProfiler :: instance()->logDebug(_D(debug_backtrace()),"EBANROT");
        }

        $args['override_item'] = $args['instance']->getOverridedItem();
        $args['use_blank_item'] = $args['instance']->isUseBlankItem();
        $this->appendDir($args);
        $this->parts[] = $args['id'];
        if (!empty($args['model_side']))
        {
            $this->parts[] = "model";
            $this->parts[] = $args['model_side'];
            $args['side'] = $args['model_side'];
            $ext = 'jpg';
        }
        elseif(isset($args['generate_type']) && $args['generate_type'] == "crop")
        {
            $this->parts[] = "crop";
        ...
        elseif(isset($args['generate_type']) && $args['generate_type'] == "cloth")
        {
            $this->parts[] = "cloth";
            $this->parts[] = "all";
            $ext = "jpg";
